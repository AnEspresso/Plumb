...
t('guide orb present, guarded, choreographed', $("!!document.getElementById('tourOrb')")===true&&await $("orbTo('#nope').then(()=>true)")===true&&await $("orbTo(null,{tap:true}).then(()=>true)")===true&&$("TOUR.grand.every(s=>Array.isArray(s.acts)&&s.acts.length)")===true&&$("TOUR.grand.filter(s=>s.acts.some(a=>a.tap)).length")===15&&$("typeof orbTaps")==='function'&&$("typeof tourPing")==='function'&&$("TOUR.grand.filter(s=>s.after).length")===7&&$("typeof tourDemoPhoto")==='function'&&$("typeof tourDemoDoc")==='function');
...
t('LAW 5: covered press moves bubble once, waits thereafter, ignores clear presses', l5ok===true&&l5a===true&&l5b===true&&l5c===false&&$("(document.getElementById('tourBubble')||{classList:{contains:()=>false}}).classList.contains('tour-bub-dock')")===true&&$("(document.getElementById('tourBubble')||{dataset:{}}).dataset.side")===$("(200>innerWidth/2)?'tl':'tr'"));
