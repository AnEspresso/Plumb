...
$('closeJobCost()');
$('openCostActual(null,"l1")');
$("setCaKind('spent')");
t('single contract on the line auto-selects as toward', $("document.getElementById('caTowardWrap').style.display")!=='none'&&$("document.getElementById('caToward').value")==='la1');
...
$('saveCostActual()');
const conPayee=$("((state.projects[0].costs||[]).find(a=>a.id==='la1')||{}).payee");
const paid=JSON.parse($("JSON.stringify((state.projects[0].costs||[]).filter(a=>a.rt==='actual'&&a.kind==='spent'&&a.toward==='la1'&&a.lineId==='l1'))"));
...
$("caLogPaymentToward('la1')");
t('one tap lands in a prefilled payment', $("_caKind")==='spent'&&$("document.getElementById('caToward').value")==='la1'&&$("document.getElementById('caPayeeAuto').style.display")!=='none');
$('closeCostActual()');

const roll=JSON.parse($("JSON.stringify(costLineRollup(state.projects[0],'l1'))"));
t('line exposure unchanged by a linked partial payment', roll.exposure===21000&&roll.spent===2500, JSON.stringify(roll));
$('openCostLine("l1")');
setVal('clBudget','25000');
$('clBudgetChanged()');
...
setVal('clExpect','26000');
$('saveCostLine()');
const line=JSON.parse($("JSON.stringify(costLines(state.projects[0]).find(l=>l.id==='l1'))"));
t('revision recorded with from, to and reason', line.revs&&line.revs.length===1&&line.revs[0].from===22000&&line.revs[0].to===25000&&line.revs[0].note==='Sim change order', JSON.stringify(line.revs));
t('expecting override stored', Number(line.expect)===26000, JSON.stringify(line.expect));
const roll2=JSON.parse($("JSON.stringify(costLineRollup(state.projects[0],'l1'))"));
t('projection follows the override', roll2.projected===26000, JSON.stringify(roll2));
$('openCostActual("la1")');
...
$('caAddSave()');
const con=JSON.parse($("JSON.stringify((state.projects[0].costs||[]).find(a=>a.id==='la1'))"));
t('add recorded on the contract', con.adds&&con.adds.length===1&&con.adds[0].amount===3000, JSON.stringify(con.adds));
const roll3=JSON.parse($("JSON.stringify(costLineRollup(state.projects[0],'l1'))"));
t('signed total includes the add', roll3.committed===24000, JSON.stringify(roll3));
$('closeCostActual()');
S('editing money never rewrites it');
$("state.projects[0].costs.push({rt:'actual',id:'zzun',lineId:'l1',kind:'spent',amount:333,payee:'Dynamite Sim',t:Date.now()})");
$("openCostActual('zzun')");
