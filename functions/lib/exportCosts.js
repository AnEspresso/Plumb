/* Export a site's logged costs to QuickBooks as Purchases.
...
async function exportCosts(db, uid, siteId) {
  ...
  costsSnap.forEach(doc => {
    const d = doc.data() || {};
    const rec = d.data;
    if (rec && rec.rt === 'actual' && Number(rec.amount) > 0) actuals.push({ docId: doc.id, rec });
  });
  ...
  for (const a of pending) {
    try {
      ...
      const note = ((a.rec.kind === 'spent' ? 'Payment' : 'Signed contract') +
        ' · ' + street + (a.rec.note ? ' · ' + a.rec.note : '') + ' · plumb:' + a.rec.id);
