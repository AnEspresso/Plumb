...
t('calendar shout says double-booked', SRC.indexOf('Double-booked ')>=0&&SRC.indexOf('function bkRenderOverlap')>=0);
...
t('label hole does not die on a zero rect', $("String(holeOpen)").indexOf('pointerEvents')>=0);
...
S('packet integrity');
t('unsigned selections cannot be marked Installed', $("(function(){state.session={role:'builder',name:'You'};state.activeId='p1';const s=(P().selections||[]).find(x=>!x.approved);if(!s)return false;const was=s.status;setSelStatus(s.id,'installed');const blocked=s.status===was&&s.status!=='installed';if(!blocked)s.status=was;return blocked;})()")===true);
t('never-signed packet cannot be marked Installed', $("(function(){state.session={role:'builder',name:'You'};state.activeId='p2';const p=state.projects.find(x=>x.id==='p2');const holdSO=JSON.stringify(p.packetSignoff||{});const holdW=JSON.stringify(p.packetWork||{});const holdSt=JSON.stringify(p.packetStale||{});p.packetSignoff={};p.packetWork={};p.packetStale={};const s=(p.selections||[]).find(x=>x.cat==='Windows'&&x.approved);if(!s)return false;const was=s.status;s.status='ordered';setSelStatus(s.id,'installed');const blocked=s.status!=='installed';s.status=was;p.packetSignoff=JSON.parse(holdSO);p.packetWork=holdW?JSON.parse(holdW):{};p.packetStale=holdSt?JSON.parse(holdSt):{};return blocked;})()")===true);
t('changing a required spec drops packet signatures', $("(function(){state.session={role:'builder',name:'You'};state.activeId='p2';const p=state.projects.find(x=>x.id==='p2');if(!packetSignedBoth(p,'windows'))return false;const s=(p.selections||[]).find(x=>x.cat==='Windows');if(!s)return false;const holdSO=JSON.stringify((p.packetSignoff||{}).windows||{});const holdSpec=JSON.stringify(s.spec||{});const holdOp=(s.spec&&s.spec.operation)||'';s.spec=Object.assign({},s.spec||{},{operation:'Changed casement'});invalidatePacketSignaturesForCat(p,'Windows');const dropped=!packetSignedBoth(p,'windows')&&packetIsStale(p,'windows');s.spec=JSON.parse(holdSpec);p.packetSignoff=p.packetSignoff||{};p.packetSignoff.windows=JSON.parse(holdSO);p.packetStale=p.packetStale||{};p.packetStale.windows=false;p.packetSpecAt=p.packetSpecAt||{};delete p.packetSpecAt.windows;return dropped;})()")===true);
t('snapshot carries signed and stale', $("(function(){const p=state.projects.find(x=>x.id==='p2');const b=(p.bookings||[])[0]||{id:'sim',trade:'windows',subName:'x',start:Date.now(),end:Date.now()};const snap=packetSnapshot(p,Object.assign({trade:'windows'},b));return 'stale' in snap&&'signed' in snap&&'work' in snap;})()")===true);
t('in-app guest packet shows stale copy', SRC.indexOf('Specs on this page are current')>=0);
t('home features the next packet and keeps capture', SRC.indexOf('Next packet')>=0&&SRC.indexOf('function nextPacketDecision')>=0&&$("String(renderToday)").indexOf('ov-field')>=0&&$("String(renderToday)").indexOf('openLogPick')>=0);
t('packet shows changes since previous approval', SRC.indexOf('Changes since previous approval')>=0&&SRC.indexOf('function packetSpecDiffHTML')>=0);
t('moving a booking date does not drop packet signatures', $("(function(){state.session={role:'builder',name:'You'};state.activeId='p2';const p=state.projects.find(x=>x.id==='p2');if(!packetSignedBoth(p,'windows'))return false;p.packetSpecAt=p.packetSpecAt||{};p.packetSpecAt.windows=tradeSpecFingerprint(p,'windows');const b=(p.bookings||[]).find(x=>x.trade==='windows')||(p.bookings||[])[0];if(!b)return false;const hold=b.start;b.start=hold+864e5;invalidatePacketSignaturesForCat(p,'Windows');const ok=packetSignedBoth(p,'windows')&&!packetIsStale(p,'windows');b.start=hold;return ok;})()")===true);
t('packet spec diff names a required field change', $("(function(){state.session={role:'builder',name:'You'};state.activeId='p2';const p=state.projects.find(x=>x.id==='p2');const s=(p.selections||[]).find(x=>x.cat==='Windows');if(!s)return false;const hold=JSON.stringify(s.spec||{});p.packetPrevSpec=p.packetPrevSpec||{};p.packetPrevSpec.windows=tradeSpecFingerprint(p,'windows');s.spec=Object.assign({},s.spec||{},{operation:'Changed casement'});const d=packetSpecDiff(p,'windows');s.spec=JSON.parse(hold);return d.some(function(c){return String(c.after).indexOf('Changed casement')>=0||String(c.field).toLowerCase().indexOf('operation')>=0;});})()")===true);
t('unsigned packets do not invent a change list', $("(function(){state.session={role:'builder',name:'You'};state.activeId='p2';const p=state.projects.find(x=>x.id==='p2');p.packetPrevSpec=p.packetPrevSpec||{};delete p.packetPrevSpec.windows;return packetSpecDiff(p,'windows').length===0&&packetSpecDiffHTML(p,'windows')==='';})()")===true);
t('fingerprint ignores prices and notes', $("(function(){state.session={role:'builder',name:'You'};state.activeId='p2';const p=state.projects.find(x=>x.id==='p2');const a=tradeSpecFingerprint(p,'windows');p.selNotes='admin chatter';const s=(p.selections||[]).find(x=>x.cat==='Windows');const hold=s.price;s.price=99999;const b=tradeSpecFingerprint(p,'windows');s.price=hold;p.selNotes='';return a===b;})()")===true);
t('no workspace overlay in this build', SRC.indexOf('SitePlumbWorkspace')<0&&SRC.indexOf('workspace.css')<0&&SRC.indexOf('Keep the build moving')<0);
t('crew can confirm signed instructions', $("(function(){const p=state.projects.find(x=>x.id==='p2');if(!packetSignedBoth(p,'windows'))return false;p.packetWork=p.packetWork||{};delete p.packetWork.windows;state.session={role:'subs',name:'Timberline Framing'};const ok=pktSetWork(p.id,'windows','ack');const acked=packetWorkAcked(p,'windows');delete p.packetWork.windows;state.session={role:'builder',name:'You'};return ok&&acked;})()")===true);
t('builder cannot confirm as the crew', $("(function(){const p=state.projects.find(x=>x.id==='p2');p.packetWork=p.packetWork||{};delete p.packetWork.windows;state.session={role:'builder',name:'You'};const blocked=!pktSetWork(p.id,'windows','ack')&&!packetWorkAcked(p,'windows');return blocked;})()")===true);
t('spec change drops crew confirm', $("(function(){state.session={role:'subs',name:'Timberline Framing'};state.activeId='p2';const p=state.projects.find(x=>x.id==='p2');if(!packetSignedBoth(p,'windows'))return false;p.packetWork=p.packetWork||{};delete p.packetWork.windows;pktSetWork(p.id,'windows','ack');const s=(p.selections||[]).find(x=>x.cat==='Windows');const holdSO=JSON.stringify((p.packetSignoff||{}).windows||{});const holdSpec=JSON.stringify(s.spec||{});s.spec=Object.assign({},s.spec||{},{operation:'Changed casement'});invalidatePacketSignaturesForCat(p,'Windows');const dropped=!packetWorkAcked(p,'windows');s.spec=JSON.parse(holdSpec);p.packetSignoff=p.packetSignoff||{};p.packetSignoff.windows=JSON.parse(holdSO);p.packetStale=p.packetStale||{};p.packetStale.windows=false;p.packetSpecAt=p.packetSpecAt||{};p.packetSpecAt.windows=tradeSpecFingerprint(p,'windows');delete p.packetWork.windows;state.session={role:'builder',name:'You'};return dropped;})()")===true);
t('signed packet cannot be marked Installed until crew confirms', $("(function(){state.session={role:'builder',name:'You'};state.activeId='p2';const p=state.projects.find(x=>x.id==='p2');p.packetWork=p.packetWork||{};delete p.packetWork.windows;const s=(p.selections||[]).find(x=>x.cat==='Windows'&&x.approved);if(!s)return false;const was=s.status;s.status='ordered';setSelStatus(s.id,'installed');const blocked=s.status!=='installed';s.status=was;return blocked;})()")===true);
...
t('guest confirm stamps the published spec fingerprint', SRC.indexOf('fp:_gpSnap.specAt')>=0||SRC.indexOf('fp:_snap.specAt')>=0);
t('rules bind guest work to specAt', (function(){try{const R=require('fs').readFileSync(require('path').join(__dirname,'../firestore.rules'),'utf8');return R.indexOf('function guestWorkOk')>=0&&R.indexOf('work.fp')>=0&&R.indexOf('resource.data.specAt')>=0;}catch(e){return false;}})());
t('stale guest work does not stamp onto the house', $("(function(){const p=state.projects.find(x=>x.id==='p2');if(!packetSignedBoth(p,'windows'))return false;const b=(p.bookings||[]).find(x=>x.trade==='windows')||{id:'sim',trade:'windows'};p.packetWork=p.packetWork||{};delete p.packetWork.windows;const fp=tradeSpecFingerprint(p,'windows');_pkStamp(p,b,{specAt:fp,signed:true,stale:false,work:{fp:'not-this-packet',ack:{at:Date.now(),by:'Crew'}}});const blocked=!packetWorkAcked(p,'windows');delete p.packetWork.windows;return blocked;})()")===true);
t('matching guest work stamps onto the house', $("(function(){const p=state.projects.find(x=>x.id==='p2');if(!packetSignedBoth(p,'windows'))return false;const b=(p.bookings||[]).find(x=>x.trade==='windows')||{id:'sim',trade:'windows'};p.packetWork=p.packetWork||{};delete p.packetWork.windows;p.packetStale=p.packetStale||{};p.packetStale.windows=false;const fp=tradeSpecFingerprint(p,'windows');_pkStamp(p,b,{specAt:fp,signed:true,stale:false,work:{fp:fp,ack:{at:Date.now(),by:'Crew'}}});const ok=packetWorkAcked(p,'windows');delete p.packetWork.windows;return ok;})()")===true);
t('missing fingerprint does not authorize work', $("(function(){const p=state.projects.find(x=>x.id==='p2');if(!packetSignedBoth(p,'windows'))return false;p.packetWork=p.packetWork||{};p.packetWork.windows={ack:{at:1,by:'Crew'},installed:{at:2},verified:{at:3}};const empty=!packetWorkAcked(p,'windows')&&!packetWork(p,'windows').ack;delete p.packetWork.windows;return empty;})()")===true);
t('reack from stale work does not keep install', $("(function(){const p=state.projects.find(x=>x.id==='p2');if(!packetSignedBoth(p,'windows'))return false;const fp=tradeSpecFingerprint(p,'windows');p.packetWork=p.packetWork||{};p.packetWork.windows={fp:'old',ack:{at:1},installed:{at:2},verified:{at:3}};state.session={role:'subs',name:'Timberline Framing'};p.packetStale=p.packetStale||{};p.packetStale.windows=false;pktSetWork(p.id,'windows','ack');const w=packetWork(p,'windows');const clean=!!(w.ack&&w.ack.at)&&!w.installed&&!w.verified;delete p.packetWork.windows;state.session={role:'builder',name:'You'};return clean;})()")===true);
t('homeowner cannot stamp the builder signature', $("(function(){const p=state.projects.find(x=>x.id==='p2');const hold=JSON.stringify((p.packetSignoff||{}).windows||{});state.session={role:'client',site:'p2',name:'Jordan'};pktSignBuilder(p.id,'windows');const same=JSON.stringify((p.packetSignoff||{}).windows||{})===hold;state.session={role:'builder',name:'You'};return same;})()")===true);
t('homeowner cannot save another house brief', $("(function(){const a=state.projects.find(x=>x.id==='p2');const b=state.projects.find(x=>x.id==='p3');if(!a||!b)return false;const hold=JSON.stringify(b.buildBrief||null);state.session={role:'client',site:'p2',name:'Jordan'};openBuildBrief('p3');const opened=_briefPid;const saved=saveBuildBrief();b.buildBrief=hold?JSON.parse(hold):undefined;state.session={role:'builder',name:'You'};return opened!=='p3'&&!saved;})()")===true);
t('unassigned crew cannot ack another trade', $("(function(){const p=state.projects.find(x=>x.id==='p2');p.packetWork=p.packetWork||{};delete p.packetWork.plumb;state.session={role:'subs',name:'Not On This House'};const blocked=!pktSetWork(p.id,'plumb','ack');state.session={role:'builder',name:'You'};return blocked;})()")===true);
t('crew cannot save the builder spec editor', $("(function(){state.session={role:'subs',name:'Timberline Framing'};state.activeId='p2';const p=state.projects.find(x=>x.id==='p2');const s=(p.selections||[]).find(x=>x.cat==='Plumbing Fixtures')||(p.selections||[])[0];const hold=JSON.stringify(s.spec||{});_specId=s.id;saveSpec();const same=JSON.stringify(s.spec||{})===hold;state.session={role:'builder',name:'You'};return same;})()")===true);
t('partial re-sign keeps the install hold and the diff', $("(function(){state.session={role:'builder',name:'You'};state.activeId='p2';const p=state.projects.find(x=>x.id==='p2');if(!packetSignedBoth(p,'windows'))return false;const s=(p.selections||[]).find(x=>x.cat==='Windows');if(!s)return false;const holdSO=JSON.stringify((p.packetSignoff||{}).windows||{});const holdSpec=JSON.stringify(s.spec||{});s.spec=Object.assign({},s.spec||{},{operation:'Polished Black'});invalidatePacketSignaturesForCat(p,'Windows');state.session={role:'client',site:'p2',name:'Jordan'};pktSignHomeowner(p.id,'windows');const html=packetHTML(p,'windows');const hold=html.indexOf('Do not install')>=0||html.indexOf('before anyone installs')>=0;const pending=!(packetSignoff(p,'windows').builder&&packetSignoff(p,'windows').builder.at);const diff=packetSpecDiff(p,'windows').length>0;s.spec=JSON.parse(holdSpec);p.packetSignoff=p.packetSignoff||{};p.packetSignoff.windows=JSON.parse(holdSO);p.packetStale=p.packetStale||{};p.packetStale.windows=false;if(p.packetPrevSpec)delete p.packetPrevSpec.windows;state.session={role:'builder',name:'You'};return hold&&pending&&diff;})()")===true);
t('guest reply notice fires before booking mutation', $("(function(){state.session={role:'builder',name:'You'};try{localStorage.removeItem(Notify._key());localStorage.removeItem(Notify._dkey());}catch(e){}const p=state.projects.find(x=>x.id==='p2');const b=(p.bookings||[]).find(x=>x.subName==='Timberline Framing')||p.bookings[0];const hold={pkResp:b.pkResp,pkQs:b.pkQs,pkQOpen:b.pkQOpen};b.pkResp=null;b.pkQs=[];b.pkQOpen=0;_pkStamp(p,b,{q:[{text:'Is the oak wrapped?',t:9}],resp:null});const n=Notify.list().some(function(x){return String(x.title||'').indexOf('asked')>=0||String(x.body||'').indexOf('oak')>=0;});b.pkResp=hold.pkResp;b.pkQs=hold.pkQs;b.pkQOpen=hold.pkQOpen;return n;})()")===true);
t('notices are namespaced by account', SRC.indexOf('plumb.notices.')>=0&&$("String(Notify._ns)").indexOf('_realOwner')>=0);
t('publisher patch carries the brief and specAt', $("String(pkPublishOne)").indexOf('brief:fresh.brief')>=0&&$("String(pkPublishOne)").indexOf('specAt:patch.specAt')>=0);
t('exit example closes the overview', $("String(exitDemoToApp)").indexOf("getElementById('overview')")>=0&&$("String(exitDemoToApp)").indexOf("classList.remove('show')")>=0);
...
t('rules keep homeowner and crew sync stamps', (function(){try{const R=require('fs').readFileSync(require('path').join(__dirname,'../firestore.rules'),'utf8');const i=R.indexOf('function rootMetaOnly');const slice=i>=0?R.slice(i,i+420):'';return slice.indexOf("'updatedAt'")>=0&&slice.indexOf("'updatedByUid'")>=0&&slice.indexOf("'data'")>=0;}catch(e){return false;}})());
t('crew cannot open the builder spec editor', $("(function(){state.session={role:'subs',name:'Timberline Framing'};state.activeId='p2';const p=state.projects.find(x=>x.id==='p2');const s=(p.selections||[]).find(x=>x.cat==='Windows')||p.selections[0];document.getElementById('specScrim').classList.remove('show');openSpec(s.id);const opened=document.getElementById('specScrim').classList.contains('show');state.session={role:'builder',name:'You'};return !opened;})()")===true);
t('second crew of the same trade can confirm', $("(function(){const p=state.projects.find(x=>x.id==='p2');if(!packetSignedBoth(p,'windows'))return false;const hold=(p.subs||[]).slice();p.subs=(p.subs||[]).concat([{id:'ink-crew-2',name:'INK Test Windows 2421',specialty:'windows'}]);p.packetWork=p.packetWork||{};delete p.packetWork.windows;p.packetStale=p.packetStale||{};p.packetStale.windows=false;state.session={role:'subs',name:'INK Test Windows 2421'};const ok=pktSetWork(p.id,'windows','ack');const acked=packetWorkAcked(p,'windows');delete p.packetWork.windows;p.subs=hold;state.session={role:'builder',name:'You'};return ok&&acked;})()")===true);
t('packet Schedule keeps the selected crew', $("(function(){state.session={role:'builder',name:'You'};state.activeId='p2';const p=state.projects.find(x=>x.id==='p2');const extra={id:'ink-crew-2',name:'INK Test Plumbing 2421',specialty:'plumb'};const hold=(p.subs||[]).slice();p.subs=(p.subs||[]).concat([extra]);openPacket(extra.id);const html=document.getElementById('infoBody').innerHTML;const keep=html.indexOf('pktGoBook')>=0&&html.indexOf('INK Test Plumbing 2421')>=0&&html.indexOf(\"pktGoBook('p2','INK Test Plumbing 2421')\")>=0;p.subs=hold;closeInfo();return keep;})()")===true);
t('deleting a house retires its guest packets', $("String(deleteSite)").indexOf('revoked:true')>=0&&$("String(deleteSite)").indexOf('siteDeleted:true')>=0);
t('guest page names a removed house', (function(){try{const P=require('fs').readFileSync(require('path').join(__dirname,'p.html'),'utf8');return P.indexOf('This house is no longer on the book')>=0&&P.indexOf('This link is no longer available')>=0;}catch(e){return false;}})());
t('standalone guest refresh signature includes specAt and brief', (function(){try{const P=require('fs').readFileSync(require('path').join(__dirname,'p.html'),'utf8');return P.indexOf('specAt:x.specAt')>=0&&P.indexOf('brief:x.brief')>=0&&P.indexOf('function guestWorkOf')>=0;}catch(e){return false;}})());
t('confirm sits above the sheet that asked', SRC.indexOf('#confirmDelScrim,#confirmScrim{z-index:var(--z-toast);}')>=0);
t('Needs You list restores after a question', SRC.indexOf('function inbHold')>=0&&$("String(nyAskClose)").indexOf('inbBackGo')>=0&&$("String(inbDraw)").indexOf('inbHold()')>=0);
t('booking from the list restores the list', $("String(closeBk)").indexOf('inbBackGo')>=0&&$("String(openBk)").indexOf('_bkBack')>=0);
t('policy Done from Legal returns to Legal', $("String(closeLegalDoc)").indexOf('_legalDocFromLegal')>=0&&$("String(openLegalDoc)").indexOf('_legalDocFromLegal')>=0);
t('closing a policy from the door does not open Legal', $("(function(){const ls=document.getElementById('legalScrim');ls.classList.remove('show');_legalDocFromLegal=false;closeLegalDoc();return !ls.classList.contains('show');})()")===true);
t('question from All Needs returns to the list', $("(function(){state.session={role:'builder',name:'You'};const p=state.projects.find(x=>x.id==='p2');const b=(p.bookings||[])[0];if(!p||!b)return false;b.pkQs=b.pkQs&&b.pkQs.length?b.pkQs:[{text:'Is the oak wrapped?'}];openNyInbox();inbHold();nyOpenAsk(p.id,b.id,0);const child=document.getElementById('nyAskScrim').classList.contains('show');const parentGone=!document.getElementById('pkLogScrim').classList.contains('show');nyAskClose();const back=document.getElementById('pkLogScrim').classList.contains('show');const childGone=!document.getElementById('nyAskScrim').classList.contains('show');closePkLog();return child&&parentGone&&back&&childGone;})()")===true);
t('Legal policy returns to Legal', $("(function(){openLegal();openLegalDoc(LEGAL_URL.privacy);const doc=document.getElementById('legalDocScrim').classList.contains('show');closeLegalDoc();const legal=document.getElementById('legalScrim').classList.contains('show');const docGone=!document.getElementById('legalDocScrim').classList.contains('show');closeLegal();return doc&&legal&&docGone;})()")===true);
...
t('timeline rail sits behind the dots', SRC.indexOf('.tl .tstep .dot{')>=0&&SRC.split('.tl .tstep .dot{')[1].slice(0,280).indexOf('position:relative')>=0&&SRC.split('.tl .tstep::before{')[1].slice(0,160).indexOf('left:-50%')>=0&&SRC.split('.tl .tstep::before{')[1].slice(0,160).indexOf('width:100%')>=0);
...
t('calendar title is Calendar', $("String(renderCal)").indexOf("'Calendar'")>=0&&SRC.indexOf('id="calHdrT">Calendar')>=0&&$("String(renderCal)").indexOf('Trades Calendar')<0);
t('calendar chips say One house and Free crews', $("String(renderCal)").indexOf('One house')>=0&&$("String(renderCal)").indexOf('Free crews')>=0&&$("String(renderCal)").indexOf('Idle / plan')<0&&$("String(renderCal)").indexOf('>Homes<')<0);
t('calendar chips are system chips', $("String(renderCal)").indexOf('class="chip')>=0&&$("String(renderCal)").indexOf('cal-chip')<0);
t('calendar twelve actions stay', ['calPick','openHomes','openIdle','openCalCrew','calMoveCancel','calMoveTo','openDay','calCrews','openBk','calMore','calNav','calToday'].every(function(n){return SRC.indexOf(n)>=0;}));
...
$('renderCal()');
t('calendar conflicts are rows when they exist', (function(){
  const body=el('calBody');if(!body)return false;
  const n=$("bookingConflicts().filter(calConflictOnScreen).filter(calConflictInMonth).length");
  const secs=[...body.querySelectorAll('.sec')].filter(function(s){return (s.textContent||'').indexOf('Needs attention')>=0;});
  if(!n)return secs.length===0;
  if(secs.length!==1)return false;
  const slice=body.innerHTML.split('Needs attention')[1].split('cal-nav')[0]||'';
  return slice.indexOf('pkt-banner')<0&&slice.indexOf('row-title')>=0&&slice.indexOf('Conflict')>=0;
})());
t('calendar has one primary', $("document.querySelectorAll('#calBody .btn-primary').length")===1);
t('calendar primary is named', (($("document.querySelector('#calBody .btn-primary')")&&$("document.querySelector('#calBody .btn-primary').textContent")||'').indexOf('Schedule a crew')>=0));
$('openBk()');
