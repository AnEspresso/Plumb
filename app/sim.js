...
t('signed selections can be marked Installed', $("(function(){state.session={role:'builder',name:'You'};state.activeId='p1';const s=(P().selections||[]).find(x=>x.approved&&x.status!=='installed');if(!s)return false;const was=s.status;setSelStatus(s.id,'installed');const ok=s.status==='installed';s.status=was;return ok;})()")===true);
