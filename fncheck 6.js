...
(async () => {
  ...
  t('export creates the pending PAYMENT only, skips the exported one', r1.ok && r1.created === 1 && r1.skipped === 1 && r1.failures.length === 0, JSON.stringify(r1));
  t('signed contracts never become Purchases', !state.purchases.some(x => x.body.PrivateNote.includes('plumb:ca2')));
  const ca2 = (await costs.doc('ca2').get()).data();
  t('contract doc untouched by export (no qbId ever)', !ca2.data.qbId && ca2.updatedBy === 'devA', JSON.stringify(ca2.data));
  t('vendor created for the paid payee only', Object.keys(state.vendors).length === 1 && !!state.vendors['Hollis Excavating']);
  ...
  t('second run creates nothing', r2.created === 0 && r2.skipped === 2, JSON.stringify(r2));
  t('mock saw exactly one purchase total', state.purchases.length === 1);
